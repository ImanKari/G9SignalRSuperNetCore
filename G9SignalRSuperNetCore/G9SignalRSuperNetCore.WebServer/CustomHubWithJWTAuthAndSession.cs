...
public class CustomHubWithJWTAuthAndSession : G9AHubBaseWithSessionAndJWTAuth<CustomHubWithJWTAuthAndSession,
    CustomClientInterface, CAuthenticationType, CustomHubSession>
{
    ...
    private static readonly G9JWTokenFactory jwToken =
        G9JWTokenFactory.GenerateJWTToken(JWTokenSecretKey, "Meti", "admin", "G9TM", "G9TM", DateTime.Now.AddDays(3));
    ...
    public override Task<G9JWTokenFactory> AuthenticateAndGenerateJwtTokenAsync(CAuthenticationType authorizeData,
        Hub accessToUnauthorizedVirtualHub)
    {
        // caller Ip Address
        // accessToUnauthorizedVirtualHub.Context.GetHttpContext()?.Connection.RemoteIpAddress
        if (authorizeData.Name ==
